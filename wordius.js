$(document).ready(function () {
  var resultSets = {
    ...
    datasets: [
      {
        backgroundColor: "rgba(0,110,220,0.5)",
        borderColor: "rgba(0,110,220,1)",
        data: [],
      },
    ],
  };
  ...
  function analyzeText() {
    ...
    if (str.length > 0) {
      ...
      $("#wordChartContainer").html(
        '<canvas id="wordChart" width="' +
          chartWidth +
          '" height="400"></canvas>'
      );
      $("#wordChartContainer");
      ...
      $("#resultSummary").html(
        "Total Unique Words: " +
          ...
          "<br />Total Word Count: " +
          totalWordCount +
          "<br />Total Word Processed: " +
          inputLength +
          "<br />Excluded Word Count: " +
          excludedCount +
          "<br />Total Words Sans Excluded Words: " +
          (totalWordCount - excludedCount) +
          "<br />Percentage of Unique Vocabulary: " +
          ((count / totalWordCount) * 100).toPrecision(8) +
          "%" +
          "<br />Percentage of Unique Vocabulary Sans Exclusions: " +
          ((count / (totalWordCount - excludedCount)) * 100).toPrecision(8) +
          "%<br /><br />"
      );
  ...
  function saveResults() {
    ...
    var chartWidth = $(window).innerWidth() - 50;
    $("#sampleChartContainer").html(
      '<canvas id="resultChart" width="' +
        chartWidth +
        '" height="400"></canvas>'
    );
